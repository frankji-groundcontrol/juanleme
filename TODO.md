...
- [ ] **1.1 项目初始化**
    - [ ] 使用 Vite 创建 Vue 3 + TypeScript 项目框架
    - [ ] 配置 TailwindCSS (安装依赖, `tailwind.config.js`, `index.css`)
    - [ ] 安装并配置 Pinia (状态管理)
    - [ ] 安装并配置 Vue Router 4 (路由管理)
    - [ ] 配置 `tsconfig.json` 路径别名 (`@/`)
    - [ ] 配置 ESLint + Prettier 代码规范
    - [ ] 提交初次 Commit
    - *负责人: 待定 | 完成日期: -*

- [ ] **1.2 基础布局与组件 (Layout & Base UI)**
    - [ ] 创建全局布局组件 `src/App.vue` (RouterView)
    - [ ] 实现导航栏组件 `src/components/layout/NavBar.vue` (响应式, 支持暗色模式切换)
    - [ ] 实现底部组件 `src/components/layout/AppFooter.vue`
    - [ ] 封装基础按钮 `src/components/common/BaseButton.vue` (多种 variant: primary, secondary, outline)
    - [ ] 封装基础输入框 `src/components/common/BaseInput.vue` (支持 label, error message)
    - *负责人: 待定 | 完成日期: -*

- [ ] **1.3 类型定义与 Mock 数据 (Types & Mocks)**
    - [ ] 定义核心类型 `src/types/user.ts` (UserProfile)
    - [ ] 定义核心类型 `src/types/workshop.ts` (Workshop, RoadmapNode)
    - [ ] 创建 Mock 服务 `src/services/mock/mockData.ts` (包含 3 个预设工作坊, 5 个节点)
    - [ ] 封装 Mock API 接口 `src/services/mock/index.ts` (模拟异步请求 delay)
    - *负责人: 待定 | 完成日期: -*
...
- [ ] **2.2 工作坊列表 (Dashboard)**
    - [ ] 实现工作坊卡片组件 `src/components/workshop/WorkshopCard.vue`
    - [ ] 实现工作坊列表页 `src/views/dashboard/WorkshopList.vue` (展示“我参与的”和“我管理的”)
    - [ ] 实现“加入工作坊”弹窗交互 (输入邀请码)
    - [ ] 实现“创建工作坊”弹窗交互 (仅组织者可见)
    - *负责人: 待定 | 完成日期: -*
